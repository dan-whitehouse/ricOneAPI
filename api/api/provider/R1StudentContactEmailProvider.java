...
import api.mapping.R1StudentContactEmailMapper;
import api.model.R1StudentContactEmail;
import api.service.R1StudentContactEmailService;

public class R1StudentContactEmailProvider extends BaseProvider 
{
	private ObjectFactory oFac = new ObjectFactory();
	private R1StudentContactEmailService studentContactEmailService = new R1StudentContactEmailService(); 
	private R1StudentContactEmailMapper mapper = new R1StudentContactEmailMapper();
	...
	{
		Collection<R1StudentContactEmail> r1StudentContactEmailes = studentContactEmailService.getStudentContactEmails(zone,context, pagingInfo);
		Collection<ContactPersonCEDSType> sifStudentContactEmailes = new ArrayList<ContactPersonCEDSType>();

		for (R1StudentContactEmail r1StudentContactEmail : r1StudentContactEmailes) 
		{
			sifStudentContactEmailes.add(mapper.modelToSIF(r1StudentContactEmail));
		}

		ContactPersonCEDSCollectionType sifStudentContactEmailCollection = oFac.createContactPersonCEDSCollectionType();
		sifStudentContactEmailCollection.getContactPersonCEDS().addAll(sifStudentContactEmailes);

		return sifStudentContactEmailCollection;
	}
	...
	{
		R1StudentContactEmail r1StudentContactEmail = studentContactEmailService.getStudentContactEmail(resourceID, zone, context);

		if (r1StudentContactEmail == null) 
		{
			return null;
		}

		ContactPersonCEDSType sifStudentContactEmail = mapper.modelToSIF(r1StudentContactEmail);
		return sifStudentContactEmail;
	}
	...
	{
		return ModelObjectConstants.c_StudentContactEmail;
	}

	@Override
	public ModelObjectInfo getMultiObjectClassInfo() 
	{
		return ModelObjectConstants.c_StudentContactEmails;
	}
